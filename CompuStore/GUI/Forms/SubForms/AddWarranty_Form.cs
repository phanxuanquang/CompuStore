...
namespace CompuStore
{
    ...
    public partial class AddWarranty_Form : Form
    {
        ...
        private void Save_Button_Click(object sender, EventArgs e)
        {
            ...
            {
                Exception res =  WarrantyServices.Instance.SaveWarrantyToDB(detail.ID_INVOICE, currentStaff.ID, Database.DataProvider.Instance.Database.PRODUCTs.FirstOrDefault(item => item.SERIAL_ID == ItemSerial_Box.Text).PRODUCT_ID, WarrantyReason.Text, DateTime.Now, WarrantyDoneDate_Picker.Value, 0);
                if (res.Message == "done")
                {
